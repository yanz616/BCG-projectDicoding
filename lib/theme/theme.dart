import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

const Color primaryColor = Color(0xff1B4242);
const Color secondColor = Color(0xff3FA796);
const Color orangeColor = Color(0xffF79329);
const Color redColor = Color(0xffD55353);
const Color priceColor = Color(0xff2C96F1);
const Color bgColor1 = Color(0xff222831);
const Color bgColor2 = Color(0xff31363F);
const Color whiteColor = Color(0xffFFFFFF);
const Color hintStyleColor = Color(0xffCDCCCD);
const Color whiteGrey = Color(0xffF3F3F3);
const Color whiteLine = Color(0xffCDCDCD);
const Color blackColor = Color(0xff181A20);
const Color greyColor = Color(0xff6A7580);

FontWeight light = FontWeight.w300;
FontWeight regular = FontWeight.w400;
FontWeight medium = FontWeight.w500;
FontWeight semiBold = FontWeight.w600;
FontWeight bold = FontWeight.w700;
FontWeight extraBold = FontWeight.w800;
FontWeight black = FontWeight.w900;

TextStyle whiteTextStyle = GoogleFonts.poppins(color: whiteColor);
TextStyle hintTextStyle = GoogleFonts.poppins(color: hintStyleColor);
TextStyle greenTextStyle = GoogleFonts.poppins(color: secondColor);
TextStyle whiteGreyTextStyle = GoogleFonts.poppins(color: whiteGrey);
TextStyle primaryTextStyle = GoogleFonts.poppins(color: primaryColor);
TextStyle greyTextStyle = GoogleFonts.poppins(color: greyColor);
TextStyle blackTextStyle = GoogleFonts.poppins(color: blackColor);
TextStyle priceTextStyle = GoogleFonts.poppins(color: priceColor);
TextStyle redTextStyle = GoogleFonts.poppins(color: redColor);
